...
from src.ML_processing.samus.prompt_encoder import PromptEncoder
...
class Samus(nn.Module):
    ...
    def forward(
          self, 
          imgs: torch.Tensor,
          pt: Tuple[torch.Tensor, torch.Tensor] = None,
          bbox: torch.Tensor = None,  # Can now be b n 4 for multiple boxes
      ) -> torch.Tensor:
          imge = self.image_encoder(imgs)
          
          # Handle multiple box prompts
          if bbox is not None:
          ...
          # Handle point prompts
              ...
              # Single set of points (original logic for 3D tensor)
              if len(pt[0].shape) == 3:
          ...
          # Handle case where neither points nor boxes are provided
              ...
              raise ValueError("Either points (pt) or bounding box (bbox) must be provided")
