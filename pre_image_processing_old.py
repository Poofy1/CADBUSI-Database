...
def caliper_box_finder(img):
    ...
    # Calculate start points for the cropping
    x_start = width - 400
    y_start = height - 100

    # Perform cropping
    img_cropped = img[y_start:height, x_start:width]
    
    # Use Canny edge detection
    edges = cv2.Canny(img_cropped, 50, 150, apertureSize=3)
    min_area = 5
    ...
    # Detect points that form a rectangle
    ...
    for cnt in contours:
        ...
        if len(approx) == 4:
            ...
            # Check if opposite sides are of approximately equal length
            if abs(side1 - side3) < 20 and abs(side2 - side4) < 20:  # 20 is a tolerance value
                area = cv2.contourArea(cnt)
                if area > min_area:
                    # Get bounding rectangle for the contour
                    x, y, w, h = cv2.boundingRect(approx)
                    
                    # Adjust x and y because of the earlier cropping
                    x += x_start
                    y += y_start
                    return True, (x, y, w, h)
...
def img_processor(img, reader, rect_US = (0,101,818,554) , debug = False, kw_list = None):
    ...
    has_calipers, rect_sizebox = caliper_box_finder(img_gray)
    ...
    img_dict['has_calipers'] = has_calipers
    ...
    for i in tqdm(image_numbers):
        ...
        if not db_out['processed'][i]:
            ...
            if file_name.endswith(('.png', '.jpg', '.jpeg', '.bmp')):
                ...
                new_features = ['crop_x', 'crop_y', 'crop_w', 'crop_h', 'has_calipers', 'description', 'is_sector', 'darkness']
                ...
                db_out.loc[i,'has_calipers'] = img_dict['has_calipers']
